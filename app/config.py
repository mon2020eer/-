...
# ---------------------------------------------------------------------------
# هوية المزوّد — **ثابتة في الشيفرة عمداً**
# ---------------------------------------------------------------------------
# هذه هوية **من يملك المنظومة ويبيعها**، لا هوية المكتب الذي يشتريها. والفرق
# بينهما جوهري ويُخلط كثيراً:
#
#   • هوية المكتب (``office_name`` وأخواتها في ``app_settings``) يضبطها المكتب
#     من شاشة الإعدادات، وهي ما **يُطبع في ترويسة العقود**: الزبون يوقّع ورقة
#     تحمل اسم المكتب الذي استأجر منه، لا اسم من كتب البرنامج.
#
#   • هوية المزوّد (هنا) تظهر **داخل البرنامج وحده** — شاشة الدخول والشريط
#     الجانبي ونافذة «عن المنظومة» — ولا تدخل ورقةً مطبوعة أبداً.
#
# وهي ثابتة هنا لا في قاعدة البيانات: ما ليس إعداداً لا توجد شاشة تغيّره، وما
# ليس في قاعدة البيانات لا يُمسح بتعديل ملف. وتُشحن داخل الملف التنفيذي.
#
# ⚠ وحدّ هذه الحماية صريح: من يملك الشيفرة المصدرية يستطيع تغيير أي سطر فيها.
#   المنعُ الحقيقي للاستعمال غير المرخَّص هو نظام المفاتيح الموقَّعة
#   (``app/core/licensing.py``) لا سطرُ اسمٍ معروض.
VENDOR_NAME = "شركة المسار المتحد"
VENDOR_TAGLINE = "تطوير وتنفيذ"
...
DEFAULT_SETTINGS = {
    # اسم **المكتب المشتري** لا اسم المزوّد: يُطبع في ترويسة عقوده ويغيّره
    # من الإعدادات. انظر ``VENDOR_NAME`` أعلاه للفرق.
...
# أنواع مصروفات السيارة في دفتر التكاليف (وحدة تحليل الربحية).
# الترتيب هنا هو ترتيب ظهورها في القائمة المنسدلة، و«أخرى» آخرها عمداً.
EXPENSE_TYPE_LABELS = {
    "maintenance": "صيانة",
    "tyres": "إطارات",
    "oil": "زيت",
...
# تصنيف ربحية السيارة كما يظهر في تقرير الأداء وفي تلوين الجدول
PROFIT_STATUS_LABELS = {
    "money_pit": "حفرة مال — الخسارة تتراكم",
    "recovering": "قيد استرداد رأس المال",
    "profitable": "مربحة — استردّت رأس مالها",
    "idle": "بلا حركة — لا عقود ولا مصروفات",
}
